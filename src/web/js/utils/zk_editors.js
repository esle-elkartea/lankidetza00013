function editor(tipo,url,column,method, autoUrl,classValidacion)
{
	this.tipo = tipo;
	this.url = url;
   ...
   if(e){ //si no esta registrado el method no hacer nada
    switch (e.getTipo()){
     case 'DATE':
	  ...
	  new Ajax.ZkInPlaceDateEditor($(id), e.getUrl(),primaryKey,grid,{});
	  break;
     case 'AUTO':
      //new Ajax.ZkInPlaceAutoCompleteEditor($(id), e.getUrl() ,primaryKey, e.getAutoUrl(),{onComplete:readValue});
	  new Ajax.ZkInPlaceAutoCompleteEditor($(id), e.getUrl() ,primaryKey, e.getAutoUrl(),grid,{});
	  break;
     case 'NORMAL':
	  //new Ajax.ZkInPlaceEditor($(id), e.getUrl() ,primaryKey,v,{onComplete:readValue}); 
	 new Ajax.ZkInPlaceEditor($(id), e.getUrl() ,primaryKey,v,grid,{}); 
	  break;
     case 'AREA':
	  //new Ajax.ZkInPlaceEditor($(id), e.getUrl() ,primaryKey,v,{onComplete:readValue,rows:4}); 
	  new Ajax.ZkInPlaceEditor($(id), e.getUrl() ,primaryKey,v,grid,{rows:4}); 
	  break;	  
     case 'FREE':
	  new Ajax.ZkInPlaceFreeLauncher($(id), e.getUrl() ,primaryKey, {});
	  break;  				   	  
     default:
      //do nothing;
      break;
